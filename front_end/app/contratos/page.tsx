...
import { HeaderPageClients } from "@/components/headerPageClient/index";
    ...
    return (
      <div className={styles["home-scope"]}>
        ...
        <main>
          ...
          {loading && <p className={styles["loading-contratos"]}>Carregando...</p>}

          {!loading && contracts.length === 0 && (
            <p className={styles["no-contratos"]}>Nenhum contrato encontrado.</p>
          )}

          {!loading && contracts.length > 0 && (
            <ul className={styles["ul-contratos"]}>
              {contracts.map((contract) => (
                <li className={styles["li-contratos"]} key={contract.id}  style={{ marginBottom: 12 ,fontSize: "1.6rem" }} >
